...
function main=getProp_1504(tab,c,htcNames,chambers)
    %% Air flows
    ...
    %Particle mass flow
    flow.mDotSand=tab.speed1*3/20;
    flow.mDotSand=round(flow.mDotSand./0.5).*0.5;     %Fix recording issue


    %Particle velocities = f(mean(eps))
    flow{:,w_pNames}=flow.mDotSand./(c.rho_p.*(1-c.eps_mf).*...
        c.l.*(bed{:,hNames}+c.hBed));
    ...
    main.w_p=mean([flow.w_p3,flow.w_p4],2);
    ...
    % main.p=flu.pBed2;
    main.p=tab.p21+tab.p19+FluBed.deltaP(bed.h4+c.hBed-c.d_t2f,...
        bed.eps2,c.rho_p);
